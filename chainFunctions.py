...
-----END PUBLIC KEY-----"""
    inf = Transaction.BlockLedger(0, "0", "0", "0", '')
    blk = BlockHeader.BlockIoTLedger(0, "0", 1465154705, inf,
...
def generateNextBlock(blockData, pubKey, previousBlock, gwPvtKey):
    ...
    inf = Transaction.BlockLedger(0, nextHash, nextTimestamp, blockData, sign)
    return BlockHeader.BlockIoTLedger(nextIndex, previousBlock.hash, nextTimestamp, inf, nextHash, pubKey);
